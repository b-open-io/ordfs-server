...
function RegisterRoutes(app) {
		...
		if (ORDFS_DOMAINS && req.hostname !== ORDFS_HOST) {
			try {
				...
				if (file.type === "ord-fs/json" && !req.query["raw"]) {
					(_a = req.res) === null || _a === void 0
						? void 0
						: _a.redirect("index.html");
					return;
				}
	...
	app.get("/v1/:network/block/height/:height", async (req, res, next) => {
		try {
			res.json(
				await (0, data_1.getBlockByHeight)(
					req.params.network,
	...
	app.get("/v1/:network/block/hash/:hash", async (req, res, next) => {
		try {
			res.json(
				await (0, data_1.getBlockByHash)(req.params.network, req.params.hash),
			);
		} catch (e) {
			next(e);
		}
	...
	async function getInscription(req, res, next) {
		...
		try {
			...
			if (file.type === "ord-fs/json" && !req.query.raw) {
				(_a = req.res) === null || _a === void 0
					? void 0
					: _a.redirect(`/${pointer}/index.html`);
				return;
			}
