...
export const byHeight = api(
    ...
    async ({ height }: { height: number }): Promise<BlockHeader> => {
        const resp = await fetch(`https://ordinals.1sat.app/v5/blocks/height/${height}`);
        if(!resp.ok) {
            throw new Error(`Failed to fetch block header: ${resp.statusText}`);
        }
        return (await resp.json()) as BlockHeader;
...
export const byHash = api(
    ...
    async ({ hash }: { hash: string }): Promise<BlockHeader> => {
        const resp = await fetch(`https://ordinals.1sat.app/v5/blocks/hash/${hash}`);
        if(!resp.ok) {
            throw new Error(`Failed to fetch block header: ${resp.statusText}`);
        }
        return (await resp.json()) as BlockHeader;
...
export const helloBitcoin = api({ expose: true, method: "GET", path: "/encore-test-bitcoin" }, async () => {
...
export const getTxRoot = api.raw(
    ...
    async (req: IncomingMessage, resp: ServerResponse) => {
        ...
        try {
            ...
            console.log(`Received raw request for root pointer: ${pointer} from req.url: ${req.url}`);
